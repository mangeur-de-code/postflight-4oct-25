...
const requiredFlightFields = [
  ...
  { key: 'aircraft_type', label: 'Aircraft Type', description: 'Select field with ACFT type.' },
  { key: 'total_flight_hours', label: 'Total Flight Hours', description: 'Select column with flight hours.' },
];
...
export default function CsvFieldMapper({ csvHeaders, onMappingComplete, onBack }) {
  ...
  const renderMappingRow = (field, isRequired = false) => {
    const missing = isRequired && !mappings[field.key];

    return (
      <TableRow key={field.key}>
        <TableCell>
          <p className={`font-medium ${missing ? "text-red-600" : "text-slate-900"}`}>
            {field.label}
            {isRequired && <span className="ml-1 text-red-600">*</span>}
          </p>
          <p className={`text-xs ${missing ? "text-red-600" : "text-slate-500"}`}>
            {field.description} {missing && "(Required)"}
          </p>
        </TableCell>
        <TableCell>
          <Select onValueChange={(value) => handleMappingChange(field.key, value)} value={mappings[field.key]}>
            <SelectTrigger className={missing ? "border-red-500 focus:ring-red-500 focus:border-red-500" : ""}>
        ...
        <div>
          ...
          <div className="border rounded-lg overflow-hidden">
            <Table>
              ...
              <TableBody>
                {requiredFlightFields.map((field) => renderMappingRow(field, true))}
              </TableBody>
            </Table>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="border rounded-lg overflow-hidden">
            <Table>
              ...
              <TableBody>
                {optionalFlightFields.map((field) => renderMappingRow(field, false))}
              </TableBody>
            </Table>
          </div>
        </div>
