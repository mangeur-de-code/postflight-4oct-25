...
export default function FlightFormNR({ flight, user, onSubmit, onCancel }) {
  ...
  const [crewType, setCrewType] = useState('rated');

  const formatUsDateInput = (value) => {
  ...
  const [formData, setFormData] = useState({
    date: flight?.date ? isoToUs(flight.date) : "",
    ...
    crew_pax: Array.isArray(flight?.crew_pax) ? flight.crew_pax : (flight?.copilot_name ? [{ name: flight.copilot_name, role: "PIC" }] : []),
    ...
    total_takeoffs: flight?.total_takeoffs || 0,
    total_landings: flight?.total_landings || 0,
    ...
    // NEW: task codes
    task_codes: Array.isArray(flight?.task_codes) ? flight.task_codes : []
  });

  const handleDateInput = (value) => {
    setFormData((prev) => ({ ...prev, date: formatUsDateInput(value) }));
  };
  ...
  useEffect(() => {
    if (flight?.pilot_role) {
      ...
      setCrewType(isNonRated ? 'non-rated' : 'rated');
    }
  }, [flight]);
  ...
  useEffect(() => {
    ...
    // This effect seems to calculate total landings for night takeoffs - which looks like a bug.
    // Assuming the intention was to calculate 'total_landings' based on 'day_landings' and 'night_landings',
    // or 'night_takeoffs' based on something else.
    // For now, preserving the original logic which sets night_takeoffs based on day_takeoffs + day_landings,
    // though this likely needs review.
    // If it was meant to be total_landings, the field names are mixed up.
  ...
  const handleCrewPaxChange = (index, field, value) => {
    const next = Array.isArray(formData.crew_pax) ? [...formData.crew_pax] : [];
    next[index] = { ...next[index], [field]: value };
    setFormData(prev => ({ ...prev, crew_pax: next }));
  };

  const addCrewPax = () => {
    const next = Array.isArray(formData.crew_pax) ? [...formData.crew_pax] : [];
    next.push({ name: "", role: "PASSENGER" });
    setFormData(prev => ({ ...prev, crew_pax: next }));
  };

  const removeCrewPax = (index) => {
    const next = (formData.crew_pax || []).filter((_, i) => i !== index);
    setFormData(prev => ({ ...prev, crew_pax: next }));
  };
  ...
  const handleSubmit = (e) => {
    ...
    let absoluteDate = "";
    if (formData.date) {
      const parsed = parse(formData.date, "MM/dd/yyyy", new Date());
      if (!isNaN(parsed.getTime())) {
        absoluteDate = formatDate(parsed, "yyyy-MM-dd");
      }
    }
  ...
  const renderFlightFields = (isSimulator = false) =>
    <div className="space-y-6">
      ...
      {/* NEW: Tail number + route moved directly under date */}
      <div className="space-y-6">
        <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <Label className="text-sm font-medium">Duty Position</Label>
          <div className="flex rounded-md bg-slate-100 p-1">
            <Button
              type="button"
              onClick={() => {
                setCrewType('rated');
                handleChange('pilot_role', 'PI');
              }}
          ...
          {/* Role checkbox grid (unchanged) */}
          <div className="space-y-2">
            <Label className="text-sm font-medium">Role</Label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {(crewType === 'rated' ? RATED_ROLES : NON_RATED_ROLES).map((role) => (
                <label
                  key={role.value}
                  className={`flex items-center gap-2 rounded-md border px-3 py-2 cursor-pointer transition-colors ${
                    formData.pilot_role === role.value ? 'bg-primary/5 border-primary/40' : 'bg-white hover:bg-slate-50'
                  }`}
                >
                  <Checkbox
                    checked={formData.pilot_role === role.value}
                    onCheckedChange={(checked) => {
                      if (checked) handleChange("pilot_role", role.value);
                    }}
                  />
                  <span className="text-sm">{role.value}</span>
                </label>
              ))}
            </div>
          </div>

          {/* REPLACED: Opposite Crew Name -> Crew / Pax list */}
          <div className="space-y-2">
            <Label className="text-sm font-medium">Crew / Pax</Label>
            <div className="space-y-2">
              {(formData.crew_pax || []).map((person, index) => (
                <div key={index} className="grid grid-cols-1 sm:grid-cols-6 gap-2 items-center">
                  <div className="sm:col-span-4">
                    <Input
                      placeholder="Name"
                      value={person.name || ""}
                      onChange={(e) => handleCrewPaxChange(index, "name", e.target.value)}
                    />
                  </div>
                  <div className="sm:col-span-2">
                    <Select
                      value={person.role || "PASSENGER"}
                      onValueChange={(val) => handleCrewPaxChange(index, "role", val)}
                    >
                      <SelectTrigger>
            ...
            <Button type="button" variant="outline" onClick={addCrewPax} className="mt-2 flex items-center gap-2">
              <Plus className="w-4 h-4" />
              Add
            </Button>
          </div>
        ...
        {/* Mission Type checkbox grid (unchanged, shows short values) */}
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {allMissionTypes.map((type) => (
              <label
                key={type.value}
                className={`flex items-center gap-2 rounded-md border px-3 py-2 cursor-pointer transition-colors ${
                  formData.mission_type === type.value ? 'bg-primary/5 border-primary/40' : 'bg-white hover:bg-slate-50'
                }`}
              >
                <Checkbox
                  checked={formData.mission_type === type.value}
                  onCheckedChange={(checked) => { if (checked) handleChange("mission_type", type.value); }}
                />
          ...
          {/* NEW: Task input visible only when Mission Type is T */}
          {formData.mission_type === "T" && (
            <div className="mt-3">
              <TaskCodesInput
                value={formData.task_codes || []}
                onChange={(codes) => handleChange("task_codes", codes)}
              />
            </div>
          )}
        </div>
